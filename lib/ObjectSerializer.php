...
class ObjectSerializer {
	 ...
	 *
	 * @param string|mixed $timestamp Original timestamp
	 *
	 * @return string the shorten timestamp
	 */
	public static function sanitizeTimestamp($timestamp): string {
		if (!is_string($timestamp)) {
			return (string)$timestamp;
		}

		return (string)preg_replace('/(:\d{2}.\d{6})\d*/', '$1', $timestamp);
	}
	...
	public static function toQueryValue(
		...
		$flattenArray = function ($arr, $name, &$result = []) use (&$flattenArray, $style, $explode) {
			...
			foreach ($arr as $key => $val) {
				$prop = ($style === 'deepObject') ? $prop = "{$name}[{$key}]" : $key;

				if (is_array($val)) {
					$flattenArray($val, $prop, $result);
				} else {
	 ...
	 * @param string[]|string[][]|null $httpHeaders HTTP headers
	 *
	...
	public static function deserialize($data, $class, $httpHeaders = null) {
		...
		if (method_exists($class, 'getAllowableEnumValues')) {
			...
			if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
				$subclass = '\BhrSdk\Model\\' . $data->{$discriminator};
	...
	*/
	public static function buildQuery(array $params, $encoding = PHP_QUERY_RFC3986): string {
		...
		$castBool = Configuration::BOOLEAN_FORMAT_INT == Configuration::getDefaultConfiguration()->getBooleanFormatForQueryString() ? function ($val) { return (int) $val;
} : function ($val) { return $val ? 'true' : 'false';
};

		$qs = '';
		foreach ($params as $key => $val) {
			$key = $encoder((string) $key);
			if (!is_array($val)) {
				$qs .= $key;
				$val = is_bool($val) ? $castBool($val) : $val;
				if ($val !== null) {
					$qs .= '=' . $encoder((string) $val);
				}
				$qs .= '&';
			} else {
				foreach ($val as $vv) {
