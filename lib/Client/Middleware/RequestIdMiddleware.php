	...
	public static function extractRequestId(ResponseInterface $response): void {
		...
		foreach ($headers as $name => $values) {
			if (strtolower($name) === $targetHeader) {
				...
				// PSR-7 ResponseInterface::getHeaders() returns string[][], so $values
				// is always an array of strings. Take the first value and coerce to
				// string for the (string|null) static property type.
				if (!empty($values)) {
